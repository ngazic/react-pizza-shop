    ...
    return (
      <div className="pizza-builder__item pizza-builder__item_mod-b">
        ...
        <div className="pizza-builder__controls-wrapper">
          <Select
            options={this.state.pizzaSizes}
            className="item-select"
            classNamePrefix="item-select"
            isSearchable={false}
            defaultValue={this.state.pizzaSizes[0]}
            onChange={this.onChange}
          />
          <button
            className="pizza-builder__btn-select ui-btn ui-btn-primary btn-effect"
            onClick={() => {
              this.props.add({
                title: this.props.title,
                size: this.state.size,
                price: this.state.price,
                image: this.state.pizzaImg,
              });
              this.animateCart();
            }}
          >
